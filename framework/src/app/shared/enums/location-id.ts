export enum LocationID {
  VOID  = 0,

  HAPPYCITY = 1,

  /****** Neigborhood  *****/

  NEIGBORHOOD = 10,

  /****** Home *****/

  HOME = 100,
  YOURROOM = 1000,
  PARENTS_ROOM= 1001,
  OLDER_SISTERS_ROOM = 1002,
  YOUNGER_SISTERS_ROOM = 1003,
  BROTHERS_ROOM = 1004,
  BATHROOM = 1005,
  LIVING_ROOM = 1006,
  KITCHEN = 1007,
  BACKYARD = 1008,
  GARAGE = 1009,

  /****** Activities Home *****/


  STRECHING_BACKYARD = 1021,
  BATH_BATHROOM = 1022,
  SHOWER_BATHROOM = 1023,
  WASH_BATHROOM = 1024,
  MEAL_KITCHEN = 1025,
  SNACK_KITCHEN = 1026,
  REST_LIVING_ROOM = 1027,
  SLEEP_YOURROOM = 1028,
  TRAIN_YOURROOM = 1029,
  STUDY_YOURROOM = 1030,
  CHARISMA_YOURROOM = 1031,
  JERKOFF_YOURROOM = 1032,
  WATCHTV_LIVING_ROOM = 1033,
  CLEAN_BATHROOM = 1034,
  CLEAN_DISHES = 1035,
  CLEAN_YOUR_ROOM = 1036,
  CUT_GLASS = 1037,
  VACUUM_LIVING_ROOM = 1038,
  WASH_CAR_GARAGE = 1039,
  PLAY_WITH_LIL_SIS = 1040,
  CHAT_WITH_MOM_KITCHEN= 1041,
  CHAT_WITH_DAD_LIVINGROOM= 1042,
  PLAY_WITH_BRO_LIVINGROOM = 1043,
  CHAT_WITH_OLD_SIS_LIVINGROOM = 1044,
  CODE_PATREON = 1045,
  FAMILY_DINNER = 1046,

  /****** DOWNTOWN  *****/

  DOWNTOWN = 20,

  /****** Cafe  *****/

  CAFE = 200,
  CAFE_BATHROOM = 2001,
  CAFE_BOSS_OFFICE = 2002,
  CAFE_TABLES = 2003,


  /****** Activities cafe  *****/

  CAFE_ASK_JOB = 2030,
  CAFE_HAND_OUT_FLYER = 2031,
  CAFE_BUY_COFFEE = 2032,
  CAFE_CHAT_CASSIE = 2033,
  CAFE_CASSIE_BJ_BATHROOM = 2034,
  CAFE_LIMPIAR_MESAS = 2035,
  CAFE_LIMPIAR_BATHROOM =2036,
  CAFE_LIMPIAR_OFFICINA_JEFE = 2037,
  CAFE_MAKE_COFFEE = 2038,
  CAFE_WAIT_TABLES = 2039,


  /****** Gym  *****/
  GYM = 210,
  GYM_SHOWER = 2101,
  GYM_WEIGHT_ROOM = 2102,
  GYM_ZOOM = 2103,




  /****** Activities GYM  *****/
BENCH_PRESS = 2121,
STRETCH_GYM = 2122,
  SHOWER_GYM =2123,
  DRINK_PROTEIN = 2124,
  BUY_GYM_COIN = 2125,


  /****** Museum  *****/
  MUSEUM = 220,

  MUSEUM_CINEMA = 2201,


  /****** Activities Museum  *****/
  PLACA1 = 2230,
  PLACA2 = 2231,
  PLACA3 = 2232,
  DONATIONS = 2234,



  /****** COMMERCIAL DISTRIC  *****/

COMMERCIAL_DISTRIC = 30,



  /****** SUPERMARKET *****/

SUPERMARKET = 300,

  /****** ACTIVITIES SUPERMARKET *****/

  BUY = 3001,
  CHAT_KIMMY = 3002,


}
